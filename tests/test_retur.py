...
def _make_completed_order(db, order_id, channel, gross=100_000, sku="SKU-1", qty=2):
    db.add(Product(sku=sku, name="Produk Uji", reference_price=gross / qty, cogs_price=10_000, stock_qty=50))
    db.commit()
    ...
    db.add(order)
    db.add(OmniOrderItem(order_id=order_id, sku=sku, item_name="Produk Uji", quantity=qty, unit_price=gross / qty))
    db.commit()
    db.refresh(order)
...
def test_process_retur_rejects_non_completed_order(db):
    ...
    try:
        process_retur(db, "SP-1", restore_stock=False)
...
def test_process_retur_rejects_double_retur(db):
    _seed(db)
    _make_completed_order(db, "SP-1", "Shopee")
    process_retur(db, "SP-1", restore_stock=False)
...
def test_process_retur_rejects_unknown_order(db):
    _seed(db)
    try:
        process_retur(db, "DOES-NOT-EXIST", restore_stock=False)
        assert False, "expected ValueError"
    except ValueError:
        pass
...
def test_shopee_retur_credits_ecommerce_receivable(db):
    _seed(db)
    _make_completed_order(db, "SP-1", "Shopee", gross=100_000)
    order = process_retur(db, "SP-1", restore_stock=False)

    assert order.status == "Dikembalikan"
    entry = db.query(JournalEntry).filter(JournalEntry.order_id == "SP-1", JournalEntry.rule_no == 19).one()
...
def test_grabfood_retur_credits_food_delivery_receivable(db):
    _seed(db)
    _make_completed_order(db, "GF-1", "GrabFood", gross=50_000)
    process_retur(db, "GF-1", restore_stock=False)

    entry = db.query(JournalEntry).filter(JournalEntry.order_id == "GF-1", JournalEntry.rule_no == 19).one()
...
def test_pos_retur_credits_the_original_nota_payment_account(db):
    ...
    db.add(Product(sku="SKU-1", name="Kopi", reference_price=20_000, cogs_price=8_000, stock_qty=50))
    db.commit()

    tunai_order = create_pos_sale(db, outlet_id="OUT-1", rule_no=8, items=[{"sku": "SKU-1", "qty": 1}])
    qris_order = create_pos_sale(db, outlet_id="OUT-1", rule_no=9, items=[{"sku": "SKU-1", "qty": 1}])

    process_retur(db, tunai_order.platform_order_id, restore_stock=False)
    process_retur(db, qris_order.platform_order_id, restore_stock=False)
...
def test_restore_stock_true_posts_rule_20_and_increases_stock(db):
    _seed(db)
    order = _make_completed_order(db, "SP-1", "Shopee", gross=100_000, sku="SKU-1", qty=2)
    product_before = db.get(Product, "SKU-1")
    ...
    process_retur(db, "SP-1", restore_stock=True)

    cogs_entry = db.query(JournalEntry).filter(JournalEntry.order_id == "SP-1", JournalEntry.rule_no == 24).one()
    recovery_entry = db.query(JournalEntry).filter(JournalEntry.order_id == "SP-1", JournalEntry.rule_no == 20).one()
    ...
    product_after = db.get(Product, "SKU-1")
    assert product_after.stock_qty == stock_before + 2


def test_restore_stock_false_skips_rule_20_and_stock_change(db):
    _seed(db)
    order = _make_completed_order(db, "SP-1", "Shopee", sku="SKU-1", qty=2)
    stock_before = db.get(Product, "SKU-1").stock_qty

    process_retur(db, "SP-1", restore_stock=False)

    assert db.query(JournalEntry).filter(JournalEntry.order_id == "SP-1", JournalEntry.rule_no == 20).first() is None
    assert db.get(Product, "SKU-1").stock_qty == stock_before


def test_process_retur_endpoint(client, db):
    _seed(db)
    _make_completed_order(db, "SP-1", "Shopee", sku="SKU-1", qty=1)
