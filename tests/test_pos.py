...
from app.models.db_models import JournalEntry, OmniOrder, Outlet, Product, Settlement
...
def test_create_pos_sale_rejects_missing_outlet(db):
    _seed(db)
    try:
        create_pos_sale(db, outlet_id="NOPE", rule_no=8, items=[{"sku": "SKU-1", "qty": 1}])
...
def test_create_pos_sale_rejects_inactive_outlet(db):
    _seed(db)
    try:
        create_pos_sale(db, outlet_id="OUT-2", rule_no=8, items=[{"sku": "SKU-1", "qty": 1}])
        assert False, "expected ValueError"
    except ValueError:
        pass
...
def test_create_pos_sale_rejects_insufficient_stock(db):
    _seed(db)
    try:
        create_pos_sale(db, outlet_id="OUT-1", rule_no=8, items=[{"sku": "SKU-1", "qty": 999}])
        assert False, "expected ValueError"
    except ValueError:
        pass
...
def test_create_pos_sale_creates_order_and_deducts_stock(db):
    _seed(db)
    order = create_pos_sale(db, outlet_id="OUT-1", rule_no=8, items=[{"sku": "SKU-1", "qty": 2}], customer_ref="Bpk. Hendra")
    ...
    product = db.get(Product, "SKU-1")
    assert product.stock_qty == 8


def test_nota_number_format_and_monthly_increment(db):
    _seed(db)
    first = generate_nota_number(db)
    assert first == f"NT-{datetime.utcnow():%Y%m}001"

    create_pos_sale(db, outlet_id="OUT-1", rule_no=8, items=[{"sku": "SKU-1", "qty": 1}])
    second = generate_nota_number(db)
    assert second == f"NT-{datetime.utcnow():%Y%m}002"


def test_tunai_payment_posts_outlet_scoped_kas_entry(db):
    _seed(db)
    order = create_pos_sale(db, outlet_id="OUT-1", rule_no=8, items=[{"sku": "SKU-1", "qty": 1}])

    entry = db.query(JournalEntry).filter(JournalEntry.order_id == order.platform_order_id, JournalEntry.rule_no == 8).one()
...
def test_qris_payment_posts_receivable_entry_without_outlet(db):
    _seed(db)
    order = create_pos_sale(db, outlet_id="OUT-1", rule_no=9, items=[{"sku": "SKU-1", "qty": 1}])

    entry = db.query(JournalEntry).filter(JournalEntry.order_id == order.platform_order_id, JournalEntry.rule_no == 9).one()
...
def test_transfer_and_ewallet_extension_rules_post_correct_accounts(db):
    _seed(db)
    transfer_order = create_pos_sale(db, outlet_id="OUT-1", rule_no=33, items=[{"sku": "SKU-1", "qty": 1}])
    ewallet_order = create_pos_sale(db, outlet_id="OUT-1", rule_no=34, items=[{"sku": "SKU-1", "qty": 1}])

    transfer_entry = db.query(JournalEntry).filter(JournalEntry.order_id == transfer_order.platform_order_id, JournalEntry.rule_no == 33).one()
    ewallet_entry = db.query(JournalEntry).filter(JournalEntry.order_id == ewallet_order.platform_order_id, JournalEntry.rule_no == 34).one()
...
def test_pos_sale_posts_cogs_entry(db):
    _seed(db)
    order = create_pos_sale(db, outlet_id="OUT-1", rule_no=8, items=[{"sku": "SKU-1", "qty": 2}])

    cogs = db.query(JournalEntry).filter(JournalEntry.order_id == order.platform_order_id, JournalEntry.rule_no == 24).one()
...
def test_get_receipt_returns_nota_ready_payload(db):
    _seed(db)
    order = create_pos_sale(db, outlet_id="OUT-1", rule_no=9, items=[{"sku": "SKU-1", "qty": 1}], customer_ref="Ibu Sari")

    receipt = get_receipt(db, order.platform_order_id)
...
def test_sales_endpoint_creates_nota_end_to_end(client, db):
    ...
    order = db.get(OmniOrder, body["no_nota"])
